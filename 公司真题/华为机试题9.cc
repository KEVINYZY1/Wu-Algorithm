...
int main() {
    ...
    while (cin >> iLen) {
        ...
        sort(R.begin(), R.end());
        map<int, set<int>> m;//R的值--符合条件I数组下标的集合
        for (int i = 0; i < R.size(); i++) {
            if (i >= 1 && (R[i] == R[i-1]))
                continue;
            set<int> tempS;//存放I[i]值
            stringstream ss;
            ss << R[i];
            string Rstring;
            ss >> Rstring;
            for (int j = 0; j < iLen; j++) {
                if (Istring[j].find(Rstring) != string::npos) {
                    if (tempS.find(I[j]) == tempS.end()) {
                        tempS.insert(I[j]);
                        m[R[i]].insert(j);
                    }
                }
